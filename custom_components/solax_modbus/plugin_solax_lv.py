...
def autorepeat_function_remotecontrol_recompute(initval: int, descr: Any, datadict: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        _LOGGER.debug(
            "[REMOTE_CONTROL] Phase currents - Measured: L1=%.2fA L2=%.2fA L3=%.2fA | House: L1=%.2fA L2=%.2fA L3=%.2fA | worst=L%s",
            measured_current_l1,
            measured_current_l2,
            measured_current_l3,
            house_current_l1,
        ...
        if remaining_current_A > 0:
            ...
            _LOGGER.debug(
                "[REMOTE_CONTROL] Phase protection (import): L%s house=%.2fA limit=%.2fA remaining=%.2fA safe_ap_target=%.1fW",
                worst_phase_idx + 1,
        ...
        _LOGGER.debug(
            "[REMOTE_CONTROL] Phase protection (export): L%s house=%.2fA (lowest) limit=%.2fA safe_ap_target=%.1fW (negative)",
            best_phase_idx + 1,
...
def autorepeat_function_powercontrolmode8_recompute(initval: int, descr: Any, datadict: dict[str, Any]) -> dict[str, Any]:
    ...
    if power_control == "Mode 8 - PV and BAT control - Duration":
        ...
        _LOGGER.debug("[Mode8 Negative Injection] probes: measured_power=%s", measured_power if measured_power is not None else "n/a")
        ...
        # Debug inputs
        _LOGGER.debug(
            "[Mode8 No-Discharge] inputs pv=%sW hl=%sW soc=%s%% max_soc=%s%% pvlimit=%sW",
            pv,
            houseload,
            battery_capacity,
            max_charge_soc,
        ...
        # Debug inputs
        _LOGGER.debug(
            "[Mode8 Export-First] inputs pv=%sW hl=%sW hl_alt=%sW (using hl) exp_lim=%sW exp_avail=%sW exp_target=%sW imp_lim=%sW soc=%s%% min_soc=%s%% max_soc=%s%% pvlimit=%sW last_push=%sW battery_charge=%sW",
            pv,
            houseload,
            houseload_alt,
            export_limit,
        ...
        _LOGGER.debug(
            "[Mode8 Export-First] probes: measured_power=%s grid_export=%s grid_import=%s",
            measured_power if measured_power is not None else "n/a",
            grid_export_s if grid_export_s is not None else "n/a",
            grid_import_s if grid_import_s is not None else "n/a",
        )
        ...
        if pv >= hl:
            ...
            _LOGGER.debug(
                "[Mode8 Export-First] export-first: surplus=%sW measured_export=%sW target=%sW error=%sW step=%sW reason=%s bms_cap≈%sW pct_cap=%sW -> charge=%sW pv_clamp_target=%sW hl=%sW",
                surplus,
                measured_export,
                export_target,
                error,
            ...
            if battery_capacity > min_discharge_soc:
                ...
                pushmode_power = 0
            _LOGGER.debug("[Mode8 Export-First] deficit: deficit=%sW soc=%s%% chosen_push=%sW", deficit, battery_capacity, pushmode_power)
        ...
        if pv < hl and pushmode_power > 0:
            ...
            if measured_export > export_deadband_w:
                ...
                _LOGGER.debug(
                    "[Mode8 Export-First] discharge feedback: -%sW (measured_export=%sW) to reduce grid export while discharging",
                    nudge,
                    measured_export,
                )
        ...
        if pv < hl:
            ...
            if pushmode_power > deficit_now:
                _LOGGER.debug("[Mode8 Export-First] clamp discharge to deficit: push=%sW -> %sW (pv=%s hl=%s)", pushmode_power, deficit_now, pv, hl)
    ...
    if old_pushmode_power != pushmode_power:
        _LOGGER.debug("import shaving: old_pushmode_power:%s new pushmode_power:%s", old_pushmode_power, pushmode_power)
...
plugin_instance = solax_lv_plugin(
    plugin_name="SolaX",
